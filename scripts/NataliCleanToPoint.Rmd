...
```{r}
( f9_long_fix = f9_dates %>% filter(long > 0) %>% mutate(across(long, ~ .x*(-1))) )
```
Grab existing negative longitude points
```{r}
( f9_long_correct = f9_dates %>% filter(long < 0) )
```
Combine correct and fixed longitude
```{r}
( f9_all_long = f9_long_correct %>% bind_rows(f9_long_fix) )
```

```{r}
f9_all_long %>% distinct(site_id, plot_id, fire_id, burn_unburn) %>% write.csv(., '../outputs/pairs/natali.csv')
```
```{r}
f9_all_long %>% filter(site_id =="YKD")
```

```{r}
f9_all_long %>% distinct(julianDate, year, date)
```
```{r}
f9_all_long %>% filter_all(any_vars(is.na(.))) 
```
...
# Save clean data
```{r}
write_csv(f9_active, "../outputs/CleanActive/NataliCleanActive.csv")
```
...
f9_pts = st_as_sf(f9_all, coords = c("long","lat"), crs = 4326, remove = FALSE)
```
...
st_write(f9_pts, "../outputs/allPoints/Natali_pts.shp", driver="ESRI Shapefile")
```