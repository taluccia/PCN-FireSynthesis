...
```
```{r}
lastnm_palette = c("#A6CEE3", "#1F78B4", "#B2DF8A", "#33A02C", "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A", "#FFFF99", "#B15928", '#1b998b', '#ed217c', '#ff9b71')
```
...
sa_map <- ggplot() +
  ...
  geom_sf(data = perma_spo_pp, fill = "#d7e8ed", colour= NA) +
   ...
   geom_sf(data = points_pp, aes(colour= last_nm)) +
  geom_sf(data=artic_pp, fill=NA, colour = "#2F4F4F", lwd=1, linetype="dashed") +
  scale_color_manual(values=lastnm_palette) +
...
(
  map2 <- ggplot() +
   ...
   geom_sf(data = sumpts_pp, aes(colour= brl_tnd)) +
  geom_sf(data=artic_pp, fill=NA, colour = "#2F4F4F", lwd=1, linetype="dashed") +
  scale_color_manual(values=brltnd_palette, name="Biome") +
    scale_fill_manual(values = permafrost_palette, labels  = c("continuous", "discontinuous", "sporatic"),
                      name = "Permafrost") +
