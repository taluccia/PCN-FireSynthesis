...
```
```{r}
unique(f12_active$date)
```
```{r}
unique(f12_active$julianDate)
```

```{r}
unique(f12_thaw$date)
```
# Save clean data
```{r}
write_csv(f12_active, "../outputs/CleanActive/OlefeldtCleanActive.csv")
```
...
f12_active_pts = st_as_sf(f12_active, coords = c("long","lat"), crs = 4326, remove = FALSE)
...
### write to shapefile
```{r}
st_write(f12_thaw_pts, "../outputs/ThawPoints/Olefeldt_thaw_pts.shp", driver="ESRI Shapefile")
st_write(f12_active_pts, "../outputs/ActivePoints/Olefeldt_active_pts.shp", driver="ESRI Shapefile")
